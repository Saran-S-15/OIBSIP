...
const CheesePage = () => {
  ...
  const { cheeses, isLoading, error, createCheese, getCheeses, deleteCheese, updateCheese } = useCheeseStore();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (edit) {
        const success = updateCheese(cheeseId, values);
        if (success) {
          setEdit(false);
          setCheeseId(null);
          formik.resetForm();
        ...
        const success = createCheese(values);
        if (success) {
          formik.resetForm();
        }
      }
    ...
    <div className='min-h-[calc(100vh-4rem)] py-10 px-4'>
      <div className='max-w-7xl mx-auto grid lg:grid-cols-2 gap-10'>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl h-fit'>
          <h2 className='text-3xl font-bold mb-6'>🧀 Add Cheese</h2>
          <form onSubmit={formik.handleSubmit} className='space-y-5'>
            ...
            >
              {
                isLoading ? <Spinner /> : edit ? "Update Cheese" : "Add Cheese"
              }
            </button>
          </form>
        </div>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl overflow-auto max-h-[calc(100vh-8rem)]'>
          <h2 className='text-3xl font-bold mb-6'>📋 Cheese List</h2>
