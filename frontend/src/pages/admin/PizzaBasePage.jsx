...
const PizzaBasePage = () => {
  ...
  const [pizzaBaseId, setPizzaBaseId] = useState(null);

  const { pizzaBases, isLoading, createPizzaBase, error, getPizzaBases, deletePizzaBase, updatePizzaBase } = usePizzaBaseStore();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (edit) {
        const success = updatePizzaBase(pizzaBaseId, values);
        if (success) {
          setEdit(false);
          setPizzaBaseId(null);
          formik.resetForm();
        ...
        const success = createPizzaBase(values);
        if (success) {
          formik.resetForm();
        }
      }
  ...
  const handleDelete = async (id) => {
    await deletePizzaBase(id);
  }

  const handleEdit = async (item) => {
    setEdit(true);
    setPizzaBaseId(item._id);
    ...
    <div className='min-h-[calc(100vh-4rem)] py-10 px-4'>
      <div className='max-w-7xl mx-auto grid lg:grid-cols-2 gap-10'>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl h-fit'>
          <h2 className='text-3xl font-bold mb-6'>🍕 Add Pizza Base</h2>
          <form onSubmit={formik.handleSubmit} className='space-y-5'>
            ...
            >
              {
                isLoading ? <Spinner /> : edit ? "Update Pizza Base" : "Add Pizza Base"
              }
            </button>
          </form>
        </div>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl overflow-auto max-h-[calc(100vh-8rem)]'>
          <h2 className='text-3xl font-bold mb-6'>📋 Pizza Base List</h2>
