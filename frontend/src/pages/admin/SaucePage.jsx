...
const SaucePage = () => {
  ...
  const { isLoading, error, sauces, createSauce, getSauces, deleteSauce, updateSauce } = useSauceStore();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (edit) {
        const success = updateSauce(sauceId, values);
        if (success) {
          setEdit(false);
          setSauceId(null);
          formik.resetForm();
        ...
        const success = createSauce(values);
        if (success) {
          formik.resetForm();
        }
      }
    ...
    <div className='min-h-[calc(100vh-4rem)] py-10 px-4'>
      <div className='max-w-7xl mx-auto grid lg:grid-cols-2 gap-10'>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl h-fit'>
          <h2 className='text-3xl font-bold mb-6'>🥫 Add Sauce</h2>
          <form onSubmit={formik.handleSubmit} className='space-y-5'>
            ...
            >
              {
                isLoading ? <Spinner /> : edit ? "Update Sauce" : "Add Sauce"
              }
            </button>
          </form>
        </div>
        ...
        <div className='bg-base-100 p-8 rounded-3xl shadow-xl overflow-auto max-h-[calc(100vh-8rem)]'>
          <h2 className='text-3xl font-bold mb-6'>📋 Sauce List</h2>
